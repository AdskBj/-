namespace TinyTeam.UI
{
    ...
    public enum UIMode
    {
        DoNothing,     //什么也不做
        HideOther,     // 关闭其他界面
        NeedBack,      // 点击返回按钮关闭当前,不关闭其他界面(需要调整好层级关系)(需要返回)
        NoNeedBack,    // 关闭TopBar,关闭其他界面,不加入backSequence队列（不需要返回）
    }
    ...
    public abstract class TTUIPage
    {
        ...
        private static Dictionary<string, TTUIPage> m_allPages;//保存当前所有UI
        ...
        private static List<TTUIPage> m_currentPageNodes;//私有保存当前图层的层级
        ...
        private static void PopNode(TTUIPage page)
        {
            ...
            bool _isFound = false;//控制记录当前页面是否添加
